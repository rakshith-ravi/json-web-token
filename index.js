...
jwt.encode = encodePromise
jwt.decode = decodePromise

function encodePromise(key, data, algorithm, cb) {
	if(cb == undefined) {
		if((typeof algorithm).toLowerCase() == 'function') {
			// callback recieved
			encode(key, data, algorithm);
		} else if(algorithm == undefined) {
			return new Promise((resolve, reject) => {
				encode(key, data, (err, token) => {
					if(err) {
						reject(err);
						cb(err, token);
						return;
			...
			return new Promise((resolve, reject) => {
				encode(key, data, algorithm, (err, token) => {
					if(err) {
						reject(err);
						cb(err, token);
						return;
...
function decodePromise(secret, token, cb) {
	return new Promise((resolve, reject) => {
		decode(secret, token, (err, decodedPayload, decodedHeader) => {
			if(err) {
				reject(err);
				cb(err, decodedPayload, decodedHeader);
				return;
			}
			resolve({
